    ...
    public static class SceneChangeHelper
    {
        ...
        public static async ETTask SceneChangeTo(Scene zoneScene, string sceneName, long sceneInstanceId)
        {
            ...
            try
            {
                ...
                var response = await session.Call(new C2M_TestActorLocationRequest() { Content = "11111111111" }) as M2C_TestActorLocationResponse;
                Log.Warning(response.Content);
                session.Send(new C2M_TestActorLocationMessage() { Content = "22222222222222222" });
            }
